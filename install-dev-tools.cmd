...
for %%p in (gcloud ripgrep fd jq yq bat gh shellcheck shfmt) do (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$p=[Environment]::GetEnvironmentVariable('Path','User')+';'+[Environment]::GetEnvironmentVariable('Path','Machine'); $env:Path=$p; if (Get-Command %%p -ea 0) { exit 0 }; scoop install %%p !PS_REDIR!; exit 0"
)
goto :eof
